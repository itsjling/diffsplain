...
plugins, or MCP settings. Diffsplain does not pass force, yolo, automatic
review, automatic MCP approval, or workspace trust flags. Cursor still
contacts its own service. An old CLI, a missing control, a trust prompt, a tool
event, or a failed canary stops Cursor before notes start. Once Diffsplain
chooses an agent, a failed check or run ends the command; it does not switch to
another agent.
