...
access denied. Cursor still contacts its own service. `--agent cursor` reports
an upgrade or boundary error before note generation when any check fails. Once
Diffsplain chooses an agent, a failed check or run ends the command; Diffsplain
does not switch agents.
