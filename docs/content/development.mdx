...
Rerun the existing workflow run or start a new run with the same exact version
after a failure. Native reruns retain the original dispatch commit and are
accepted when that commit is the parent of the pushed release. The workflow
accepts only these states:
