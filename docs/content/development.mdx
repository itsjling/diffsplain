...
The workflow installs the pinned toolchain, validates its GitHub OIDC context,
runs the full release verification, pushes the exact version commit and tag
atomically, and then publishes the verified tarball. Stable versions use the
`latest` npm tag and prereleases use `next`. Releases are serialized so two
workflow runs cannot publish concurrently.

### Recover a workflow release

Rerun the workflow with the same exact version after a failure. It accepts only
these states:
